...
export class GoogleSpreadsheetService {
   ...
   */
    ...
    //     const shiftsByDate = {};
    //     rows.forEach(row => {
    //       const [day, month, year] = row._rawData[1].split("/");
    //       const formattedDate = `${year}-${month.padStart(2, '0')}-${day.padStart(2, '0')}`; // Asegúrate de que el formato sea YYYY-MM-DD para ordenar
    //       if (shiftsByDate[formattedDate]) {
    //         shiftsByDate[formattedDate].push(row);
    //       } else {
    //         shiftsByDate[formattedDate] = [row];
    //       }
    //     });
        
    //     const sortedDates = Object.keys(shiftsByDate).sort((a, b) => new Date(a).getTime() - new Date(b).getTime());
        
    //     // Aquí es donde se busca el primer día con menos de 4 turnos registrados.
    //     let nextAvailableDate = sortedDates.find(date => shiftsByDate[date].length < 4);
        
    //     // Si no se encuentra tal día (es decir, todos los días tienen 4 turnos),
    //     // se procede a calcular el día siguiente al último registrado.
    //     if (!nextAvailableDate) {
    //       const lastRegisteredDay = new Date(sortedDates[sortedDates.length - 1]);
    //       const nextDay = new Date(lastRegisteredDay.setDate(lastRegisteredDay.getDate() + 1));
    //       nextAvailableDate = nextDay.toLocaleDateString('es-PE', {
    //         timeZone: 'America/Lima',
    //       });
    //     } else {
    //       // Si se encuentra un día con menos de 4 turnos, se usa esa fecha directamente.
    //       const dateObj = new Date(nextAvailableDate); // Convert the string to a Date object
    //       console.log('dateObj', dateObj);
    //       nextAvailableDate = dateObj.toLocaleDateString('es-PE', {
    //         timeZone: 'America/Lima',
    //       });
    //     }

    //     // Formatear la fecha encontrada o calculada a DD/MM/YYYY
    //     const formattedDateDDMMYYYY = nextAvailableDate;
